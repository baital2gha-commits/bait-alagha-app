...
let sliderInterval; // تعريف المتغير هنا للتحكم به عالمياً
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupMainTabs(); // تفعيل التنقل الرئيسي
    ...
    if (locBtn) locBtn.onclick = () => { getLocation(); };
    if (submitBtn) submitBtn.onclick = () => { sendOrderToWhatsApp(); };
});

// --- دالة التحكم في التبويبات والسلايدر التلقائي ---
function setupMainTabs() {
    ...
    mainTabButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // تحكم ذكي في السلايدر: يبدأ فقط عند فتح تبويب "بيت الآغا"
    ...
    // إيقاف السلايدر عند التفاعل اليدوي
    if (sliderTrack) {
        sliderTrack.addEventListener('touchstart', stopAutoSlider);
        sliderTrack.addEventListener('mousedown', stopAutoSlider);
        sliderTrack.addEventListener('touchend', () => setTimeout(startAutoSlider, 5000));
    }
}

function startAutoSlider() {
    stopAutoSlider(); // مسح أي عداد قديم لتجنب السرعة المضاعفة
    const sliderTrack = document.querySelector('.slider-track');
